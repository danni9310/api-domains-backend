...
func OrganizeAnswer(body string, logo string, title string, serverChanged bool, previousGrade string) DomainInfoSend {
...
func ValidateConditions(db *sql.DB, data AnswerRequest) DomainInfoSend {
	...
	if data.RequestHash == "" {
		...
		result = OrganizeAnswer(string(data.Body), data.Logo, data.Title, false, "")
		...
		if timeInterval > hour {
			...
			if bodyHashed != data.RequestHash {
				result = OrganizeAnswer(string(data.Body), data.Logo, data.Title, true, "")
				domains.Update(db, data.Name, bodyHashed, timeNow, result.Ssl_grade)
				fmt.Println("Change server")
			} else {
				result = OrganizeAnswer(string(data.Body), data.Logo, data.Title, false, data.PreviousGrade)
...
func GetDomainParameters(ctx *fasthttp.RequestCtx) {
	...
	defer func() {
		if err != nil {
			...
			utils.DoJSONWrite(ctx, http.StatusNotFound, err)
		}
	}()

	var data AnswerRequest
	name := strings.ToLower(fmt.Sprintf("%s", ctx.UserValue("name")))
	data.Name = name
	...
	utils.DoJSONWrite(ctx, http.StatusOK, result)
